...
def division_generator(a, b, n, include_last=True):
    ni = int(n)
    assert(ni == n)
    d = b - a
    for i in range(ni):
...
def division(a, b, n, include_last=True):
    return list(division_generator(a, b, n, include_last))

def map_division(f, a, b, n, include_last=True, *rest):
    if len(rest) == 0:
        return list([f(x) for x in division_generator(a, b, n, include_last)])
    elif len(rest) == 2:
        return [[f(x, y) for y in division_generator(include_last, *rest)]
                for x in division_generator(a, b, n, True)]
    elif 3 <= len(rest) <= 4:
        return [[f(x, y) for y in division_generator(*rest)]
                for x in division_generator(a, b, n, include_last)]
    elif 6 <= len(rest) <= 8:
        t0 = rest[:4]
        rest1 = rest[4:]
        a1, b1, n1, include_last1 = t0
        return [[[f(x, y, z) for z in division_generator(*rest1)]
                 for y in division_generator(a1, b1, n1, include_last1)]
                for x in division_generator(a, b, n, include_last)]
    else:
...
def frange(start=0, stop=None, step=1):
    ...
    if isinstance(start, int) and isinstance(stop, int) and isinstance(step, int):
        return list(old_range(start, stop, step))
    else:
...
def map(func, lst, *lsts):
    return list(old_map(func, lst, *lsts))
    
old_filter = filter
def filter(pred, lst, *lsts):
    return list(old_filter(pred, lst, *lsts))
    

old_zip = zip
def zip(lst, *lsts):
    return list(old_zip(lst, *lsts))
